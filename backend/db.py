...
import os, json, sqlite3, time, io, csv, hashlib, secrets
...
def _ensure_schema_migrations(c: sqlite3.Connection):
    if not _col_exists(c, "pendings", "conversation_id"):
        try: c.execute("ALTER TABLE pendings ADD COLUMN conversation_id TEXT")
        except Exception: pass
    if not _col_exists(c, "orders", "phone"):
        try: c.execute("ALTER TABLE orders ADD COLUMN phone TEXT")
        except Exception: pass
    if not _col_exists(c, "orders", "delivery_type"):
        try: c.execute("ALTER TABLE orders ADD COLUMN delivery_type TEXT")
        except Exception: pass
    if not _col_exists(c, "orders", "sla_deadline"):
        try: c.execute("ALTER TABLE orders ADD COLUMN sla_deadline TEXT")
        except Exception: pass
    if not _col_exists(c, "orders", "sla_breached"):
        try: c.execute("ALTER TABLE orders ADD COLUMN sla_breached INTEGER NOT NULL DEFAULT 0")
        except Exception: pass
    ...
    if seed:
        ...
        if cur.fetchone()["n"] == 0:
            for (nm, desc, pr) in [
                ...
                ("Postre", "Brownie de chocolate", 3.25),
            ]:
                try:
                    cur.execute("INSERT INTO menu_items(name, description, price, currency, special_notes) VALUES (?,?,?,?,?)",
                        (nm, desc, pr, get_config().get("currency","USD"), ""))
                except Exception:
                    pass
        cur.execute("SELECT COUNT(*) AS n FROM tenants")
        if cur.fetchone()["n"] == 0:
            cur.execute("INSERT INTO tenants(name, slug) VALUES (?,?)", ("Demo Restaurant", "demo"))
        ...
        if cur.fetchone()["n"] == 0:
            ...
            faqs = [
                (tenant_id, "es", r"horario|abren|cierran", "Nuestro horario es de 11:00 a 22:00, todos los días."),
                (tenant_id, "es", r"\bdelivery\b|domicilio", "Hacemos delivery en un radio de 5 km. Costo según distancia."),
                (tenant_id, "en", r"hours|open|close", "We open 11:00 to 22:00, every day."),
                (tenant_id, "en", r"delivery", "We deliver within 5 km radius. Cost varies by distance."),
            ]
            cur.executemany("INSERT INTO faqs(tenant_id, language, pattern, answer) VALUES (?,?,?,?)", faqs)
    c.commit(); c.close()

# Menu
def fetch_menu() -> List[Dict[str, Any]]:
    c = _conn()
    rows = c.execute("SELECT id, name, description, price, currency, special_notes FROM menu_items ORDER BY id ASC").fetchall()
    c.close()
...
def add_menu_image(file):
    ...
    filename = getattr(file, "name", f"img_{ts}.png")
    if "." not in filename:
...
def fetch_menu_images() -> List[str]:
    c = _conn()
    rows = c.execute("SELECT file_path FROM menu_images ORDER BY id DESC").fetchall()
    c.close()
...
def create_order_from_chat_ready(client: Dict[str, Any], items: List[Dict[str, Any]], currency: str) -> Dict[str, Any]:
    ...
    cfg = get_config()
    sla_deadline = (datetime.utcnow() + timedelta(minutes=int(cfg.get("sla_minutes", 30)))).isoformat()
    row = {
        "id": order_id,
...
def create_pending_question(conversation_id: str, question: str, language: str, ttl_seconds: int = 60):
    ...
    c = _conn()
    c.execute("""INSERT INTO pendings(id, conversation_id, question, language, created_at, expires_at, status, answer)
                 VALUES(:id,:conversation_id,:question,:language,:created_at,:expires_at,:status,:answer)""", row)
    c.commit(); c.close()
    return row

def fetch_pending_questions() -> List[Dict[str, Any]]:
    c = _conn()
    rows = c.execute("SELECT * FROM pendings WHERE status = 'pending' ORDER BY expires_at ASC").fetchall()
    c.close()
    return [dict(r) for r in rows]

def has_pending_for_conversation(conv_id: str) -> bool:
    c = _conn()
    rows = c.execute("SELECT COUNT(*) AS n FROM pendings WHERE conversation_id = ? AND status = 'pending'", (conv_id,)).fetchone()
    c.close()
    return (rows["n"] or 0) > 0

def answer_pending_question(pending_id: str, status: str, answer: str = ""):
    c = _conn()
    c.execute("UPDATE pendings SET status = ?, answer = ? WHERE id = ?", (status, answer, pending_id))
    c.commit(); c.close()
...
def export_pendings_csv() -> str:
    c = _conn()
    rows = c.execute("SELECT * FROM pendings ORDER BY created_at DESC").fetchall()
    c.close()
    output = io.StringIO()
    writer = csv.writer(output)
    if rows:
...
def list_faqs(tenant_id: Optional[int], lang: str) -> List[Dict[str,Any]]:
    c = _conn()
    if tenant_id:
        rows = c.execute("SELECT * FROM faqs WHERE tenant_id = ? AND language = ? ORDER BY id ASC", (tenant_id, lang)).fetchall()
    else:
        rows = c.execute("SELECT * FROM faqs WHERE tenant_id IS NULL AND language = ? ORDER BY id ASC", (lang,)).fetchall()
    c.close()
    return [dict(r) for r in rows]
