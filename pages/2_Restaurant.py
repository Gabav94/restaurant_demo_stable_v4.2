...
with col1:
    ...
    with st.form("add_item"):
        ...
        price = st.number_input(t("Precio","Price"), min_value=0.0, value=0.0, step=0.1)
...
with col2:
    ...
    up = st.file_uploader(t("Subir CSV/TXT","Upload CSV/TXT"), type=["csv","txt"])
...
if view == t("Tabla","Table"):
    menu_table_component(menu, lang, deletable=True, on_delete=delete_menu_item)
else:
    gallery = fetch_menu_images()
    if not gallery:
        st.info(t("No hay imágenes cargadas aún.","No images uploaded yet."))
    else:
        render_js_carousel(gallery, interval_ms=5000, aspect_ratio=16/6, key_prefix="rest_menu", show_dots=True, height_px=520)
...
with c1:
    ...
    if not orders:
        ...
        try: st.dataframe(df, hide_index=True, width='stretch')
        except TypeError: st.dataframe(df, hide_index=True)
...
with c2:
    st.subheader(t("Interacciones por confirmar (1 min)","Pending interactions (1 min)"))
